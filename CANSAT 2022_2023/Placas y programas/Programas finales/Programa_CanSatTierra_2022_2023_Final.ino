 ...
 * - 03/05/23 - Versiones 1.6 y 1.7: Cambiadas las fórmulas de distancias. 
 * - 04/05/23 - Versión 2.0: Todo finalizado. Aún hay cosass que no entiendo, pero así se quedan. Paso de todo :D.
 */ 
...
const unsigned char logo [] PROGMEM = {                                                                     //Array de bits para las distintas imagenes.
...
void showData(char* text1, float var1, char* unidad1, char* text2, float val2, char* unidad2, bool flecha = true){  //Función showData(), que nos permite ahorrar lineas de código y mostrar los valores más facilmente.
  ...
  if(sdInsertada == true){                                                                                  //Mostramos un icono o otro según el estado de la SD. 
  ...
  display.print(lastRSSI);                                                                                  //Mostramos le última potencia recibida. 
  display.setCursor(97, 55);
  display.print(gps.time.hour());
  display.print(":");
  display.println(gps.time.minute());                                                                       //Mostramos la hora abajito.
  ...
  display.drawCircle(2, 52, 2, WHITE);                                                                      //Círculos vacíos para los menús.
  switch (screen){                                                                                          //Rellenamos el círculo que corresponda según la pantalla. 
    case 0:
  ...
  display.display();                                                                                        //Actualizamos la pantalla siempre para que se muestren los cambios. 
}
...
void setup() {
 ...
 |-----------------------------------------------------------------------------------|)");                  //Mensaje de bienvenida (Serial)
  Wire.begin(21,22);                                                                                        //Conexión WIRE para la OLED.
  EEPROM.begin(EEPROM_SIZE);                                                                                //Inicializamos la EEPROM (Preferencia de pantalla y antena).
  ...
  pinMode(boton, INPUT);                                                                                    //Botón inicializado.
  LoRa.setPins(18, 23, 26);                                                                                 //Pines del LoRa (Internos).
  ...
  hspi = new SPIClass(HSPI);                                                                                //Inicialización de la SD.
 ...
 |-----------------------------------------------------------------------------------|)");                  //Cierra mensaje inicial. 

  invertDisplay = EEPROM.read(1);                                                                           //Ponemos como preferencia la pantalla que esté seleccionada.
  display.clearDisplay();                                                                                   //Limpiamos la pantalla por si acaso.
  display.setTextColor(WHITE);
  display.invertDisplay(invertDisplay);
  display.drawBitmap(90, 5, logo, 30, 30, WHITE);                                                           //Mostramos el logo.
  display.setTextSize(2);
  ...
  display.write(162);                                                                                       //Esto es para poder mostrar la tilde. 
  display.print("n de tierra");
  display.setCursor(100, 56);
  display.print("v");
  display.print(VERSION, 1);                                                                                //Versión del cansat
  display.display();                                                                                        //Mostramos la pantalla. 
  delay(1000);
  display.clearDisplay();                                                                                   //Limpiamos la pantalla para mostrar ya lo próximo.
  display.display();


  archivo = SD.open("/data.csv", FILE_WRITE);                                                               //Abrimos el archivo en la SD y comprobamos si ya tenía datos escritos.
  archivo.seek(archivo.size());                                                                             //Nos movemos hasta la última posición del texto.
  if(archivo.size() > 10){                                                                                  //Si el tamaño es mayor de 10 bytes, quire decir que ya hay algo escrito, por lo que imprimimos mensaje en el Serial
    Serial.println(" |!| La tarjeta SD tiene datos anteriores a este encendido.");                          //y en la propia SD para advertir.
    ...
    archivo.print("0");
    archivo.print(';');
    archivo.print('\n');                                                                                    // |!| NO CAMBIAR |!| NO SE PORQUE NO FUNCIONA.
    archivo.close();
  }
  else{                                                                                                     //Si no, escribimos los headers de cada sección.
...
void loop() {
 ...
 |----------------------------------------------------------------|)");
  ...
  valores[7] = distancia(lastRSSI);                                                                         //Llamamos a la función "distancia" para calcular la distancia real.
  if(saved == false){                                                                                       //Si no está guardado, lo guardamos. 
    archivo = SD.open("/data.csv", FILE_WRITE);                                                             //Abrimos el documento.
    if(archivo){                                                                                            //Si se abre bien, guardamos toda la morralla.
  ...
  saved = true;                                                                                             //Actualizamos la variable "saved" (Ya está guardado).
  }
  ...
  if(digitalRead(boton) == LOW){                                                                            //Si lee el boton, cambia de pantalla. (El botón esta pull-up).
    if(botonPulsado == false){                                                                              //Actualizamos la variable de "boton pulsado"
      botonPulsado = true;
      tiempoPrevBoton = tiempo;                                                                             //Guardamos el momento en el que se presionó el botón.
    }
    if((tiempo - tiempoPrevBoton > 1250) && (botonPulsadoLargo == false) && (screen == 5)){                 //Si el tiempo es mayor que 1,25 segundos, y no se ha reconocido una "pulsación larga", 
      botonPulsadoLargo = true;                                                                             //y estamos en la pantalla nº 5,
      antenna = antenna + 1;                                                                                //Actualizamos el valor de "botonPulsadoLargo" y el valor de la antena seleccionada.
      if(antenna > 3){
        antenna = 1;
      }
      EEPROM.write(0, antenna);                                                                             //Guardamos el valor de la antena nueva en la EEPROM por si se apaga/reinicia.
      EEPROM.commit();                                                                                      //Guardamos los cambios en la EEPROM.
      Serial.print(" |a| Antena seleccionada: ");
      Serial.println(antenna);                                                                              //Imprimimos la antena seleccionada por el Serial.
      botonPulsado = false;                                                                                 //Una vez se ha hecho todo, el botón ya no "Está pulsado"
    }
  }
  else{
     if(botonPulsado == true){                                                                              //Si el botón "está pulsado", ya no.
      if(botonPulsadoLargo == true){
        botonPulsadoLargo = false;                                                                          //Lo mismo pero con la variable de pulsación larga.
      }
      else{                                                                                                 //Si no es pulsación larga, es corta:
        screen = screen + 1;                                                                                //Pasamos de pantalla.
        if(screen > 5){
          screen = 0;
        }
        Serial.print(" |o| Pantalla: ");
        Serial.println(screen);                                                                             //Lo mostramos por el Serial.
        delay(200);                                                                                         //Pequeño delay.
      }
      botonPulsado = false;                                                                                 //El botón ya no está pulsado.
    }
  }
...
void descifrarCadena(){
  int packetSize = LoRa.parsePacket();                                                                      //Obtenemos el tamaño del paquete disponible.
    if (packetSize) {                                                                                       //Si hay algo,
      Serial.print(" |>| Cadena recibida: ");                                                               //Mostramos mensaje por el serial,
      cadenaRecibida = "";                                                                                  //Reseteamos la varibale de la cadena entera,
      while (LoRa.available()) {                                                                            //Y mientras haya algo para leer, vamos "sumándolo" a la variable de la cadena entera.
        cadenaRecibida = cadenaRecibida + (char)LoRa.read();
      }
      mostrado = false;                                                                                     //Si hay datos nuevos, quiere decir que no están mostrados aún.
      Serial.println(cadenaRecibida);                                                                       //Imprimimos la cadena entera por el serial, para comprobar cualquier fallo.
      lastRSSI = abs(LoRa.packetRssi());                                                                    //Obtenemos la última potencia de señal, y la convertimos con valor absoluto.
      Serial.print(" |>| Potencia de la cadena: ");
      Serial.println(lastRSSI);                                                                             //Imprimimos la potencia.
      cadenaNew = true;                                                                                     //Hay cadena nueva, así que actualizamos.
      saved = false;                                                                                        //Si es nueva, no se ha guardado en la SD
      tiempoPrev = tiempo;                                                                                  //Variable de control del tiempo para la flechita y tal.

      display.display();                                                                                    //Atualizamos la pantalla (?).
    }
    cadaParte = parte(parte(cadenaRecibida,'$',1),'#',0);                                                   //Elimina el caracter final e inicial., ya que son marcadores. 
    if ((cadaParte.length()>0) || (cadenaRecibida != cadenaAntigua)){                                       //Si sigue habiendo algo (La cadena no estaba vacía), 
 	    ...
 	    i = 0;
 	    while((parte(cadaParte,'?',i).length()>0) && (mostrado == false)) {                                   //No entiendo esto.
        //Serial.println(parte(cadaParte,'?',i)+ " --> "+(String) i);
        valores[i] = parte(cadaParte,'?',i).toFloat();                                                      //Guarda los datos en las variables.
        i++;
 	    }
       mostrado = true;                                                                                     //Esto no se porque está aquí
    }
    cadaParte="";                                                                                           //Limpia la variable.
    cadenaAntigua = cadenaRecibida;                                                                         //Actualiza la cadena antigua.
}

float distancia(int potencia){                                                                              //Función para obtener la distancia
  float d;                                                                                                  //Float para la distancia 
  switch (antenna){                                                                                         //Según la antena seleccionada, se usa una ecuación u otra.
    case 1:
  ...
  return(d);                                                                                                //Devolvemos el valor de d para lo que se necesite.
}
