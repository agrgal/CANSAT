 ...
 * - 03/05/23 - Versión 2.0: Rematado todo. Añadida la comprobación de altura antes de abrir el servo. Limpiado de código. 
 * - 04/05/23 - Versión 2.1: Últimos ajustes de tiempos. Proyecto listo. 
 */ 
...
#define ALTURADESCENSO 2
#define version "2.1"                                                                                       //Usamos #define para ahorrar variables innecesarias pero seguir podiendo cambiar los valores facilmente a la hora de editar.
...
float altura = 0;
float alturaPrev = 0;                                                                                       //Altura previa, para calcular cuándo mover el servo.
int cayendo;
...
void leerGY87(){
  ...
  aceTotal = sqrt((pow(ace[0], 2)) + (pow(ace[1], 2)) + (pow(ace[2], 2)));                                  //Calculamos la aceleración total. 
}

void crearCadena(){
  char cadena [48] = "";
  snprintf(cadena, sizeof(cadena), "$%ld?%.2f?%.2f?%.2f?%.2f?%d?%.2f#", ncadena, ang[0], ang[1], aceTotal, temperatura, presion, altura); //Formateamos los valores a enviar en 1 sola cadena.
...
void loop() {
 ...
 |--------------------------------------------|)");
  ...
  if((alturaPrev >= (altura + ALTURADESCENSO)) && (tiempo >= tiempoPrevBucle + LOOPTIME)){                               //Si la altura anterior es igual o mayor que la altura nueva mas 1m, quire decir que estamos cayendo, por lo que aumentamos 1 al contador
    cayendo = cayendo + 1;                                                                                  //(Contador para comprobar que estamos cayendo 2 veces mínimo, y evitar posibles errores.)
