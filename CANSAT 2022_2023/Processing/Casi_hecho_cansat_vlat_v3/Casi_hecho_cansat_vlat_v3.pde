...
int numfilas = 21;
int numfilas2 = 45;//numero de filas en el archivo()
...
float[] rotX = new float [numfilas2];
float[] rotY = new float [numfilas2]; 
float[] temp = new float [numfilas2];
...
//print(minz);
  ...
  tempyrot = loadTable("data1.csv","header");//Nomyre del archivo

  for (int k = 0; k<tempyrot.getRowCount(); k++) {//Metemos los numeros del archivo a unas listas
    rotY[k] = tempyrot.getFloat(k, "Ángulo en x");
    rotX[k] = tempyrot.getFloat(k, "Ángulo en y"); 
    temp[k] = tempyrot.getFloat(k, "Temperatura");
  }
  println(temp);
...
void draw() {
  ...
  for ( int n = 0; n < numfilas; n = n+1) {//Diyujo de los puntos
    ...
    fill(lerpColor(color(0, 0, 255), color(255, 0, 0), map(temp[n],mintemp, maxtemp, 1, 100)/100));//Color puntos
